...
int main() {
    ...
    // Smooth the path
    std::vector<Point> smoothed_path = smooth_path(path, 0.5, 0.5, 10);
    ...
    std::vector<double> x_smoothed_path;
    std::vector<double> y_smoothed_path;
    for (const auto& p : smoothed_path) {
        x_smoothed_path.push_back(p.x);
        y_smoothed_path.push_back(p.y);
    }
    plt::plot(x_smoothed_path, y_smoothed_path, "bo-");
